...
@php
    $isEdit = $mode === 'edit';
    $excursionType = $excursion->eidos_ekdromis ?? null;
...
<div class="bg-white rounded-lg shadow-md overflow-hidden" x-data="{ selectedType: '{{ $excursionType ?? '' }}' }">
    ...
    <div class="p-6">
        ...
        <form action="{{ $isEdit ? route('excursion.update', $excursion) : route('excursion.store') }}" method="POST">
            ...
            {{-- Type Selector --}}
            <div class="mb-6">
                <label for="eidos_ekdromis" class="block text-sm font-medium text-gray-700 mb-1">Είδος Εκδρομής *</label>
                @if($isEdit)
                ...
                @else
                    <select name="eidos_ekdromis" id="eidos_ekdromis" x-model="selectedType"
                            class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-coral focus:border-transparent" required>
            ...
            @php
                $generalFields = [
                    'ar_prajis_syllogou', 'a_arithmos', 'proorismos', 'onoma_jenodoxeio', 'onoma_praktoreio',
                    'metaforika_mesa', 'mathimata', 'tmimata', 'titlos_programmatos', 'eidos_programmatos',
                    'ar_pr_egrisis_programmatosdde', 'asf_symbolaio', 'praji_epilogi_praktoreiou', 'ar_pr_anartisisprok',
                    'erasmus_ar_simbasis', 'erasmus_ar_prajis_syllogou_sigrotisi', 'erasmus_ar_prajis_syllogou_anasigrotisi',
                    'erasmus_ar_prajis_syllogon_sinainesi', 'erasmus_ar_prot_beb_dieythinton',
                ];
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach($generalFieldsToShow as $fieldName)
                        ...
                        <div x-show="[{{ $showFor }}].includes(selectedType)" x-transition
                            ...
                            @if($isEdit)
                                @include('excursion._field-edit', ['fieldName' => $fieldName, 'fieldDef' => $fieldDef, 'excursion' => $excursion])
                            @else
                                @include('excursion._field-create', ['fieldName' => $fieldName, 'fieldDef' => $fieldDef])
                            @endif
                        </div>
                    @endforeach
                </div>
            ...
            {{-- Dates Section --}}
            @php
                $dateFields = ['hmera_ekdromis_anaxorisis', 'hmera_epistrofis', 'diarkeia_hmeres', 'ora_anaxorisis', 'ora_afijis', 'ora_apoxorisis', 'ora_epistrofis'];
                    ...
                    @foreach($dateFieldsToShow as $fieldName)
                        ...
                        <div x-show="[{{ $showFor }}].includes(selectedType)" x-transition>
                            @if($isEdit)
                                @include('excursion._field-edit', ['fieldName' => $fieldName, 'fieldDef' => $fieldDef, 'excursion' => $excursion])
                            @else
                                @include('excursion._field-create', ['fieldName' => $fieldName, 'fieldDef' => $fieldDef, 'required' => $fieldName === 'hmera_ekdromis_anaxorisis'])
                            @endif
                        </div>
                    @endforeach
                </div>
            ...
            @php
                $partFields = ['ar_mathiton', 'ar_metakinoumenon', 'onoma_arxigos', 'plithos_synodoi', 'plithos_ektosomadas_synodoi', 'erasmus_lista_kathig_kaieidikotita', 'erasmus_lista_anaplirkathig_kaieid', 'erasmus_lista_mathites_kaitaji'];
