...
@section('content')
    ...
    <ol>
        ...
        @endphp
        <li><b>{{ $excursion->ar_metakinoumenon }}</b> μαθητές και μαθήτριες
            του σχολείου μας και <b>{{ $plithos_ekp }}</b> εκπαιδευτικοί πρόκειται
            να μετακινηθούν με τον εξής προορισμό: <b>{{ $excursion->proorismos }}</b>
            για την εξής εκδήλωση: <b>{{ $excursion->titlos_programmatos }}</b>,
            {{ $hmerominies }} στο πλαίσιο του άρθρου 8.
        </li>
        <li>
