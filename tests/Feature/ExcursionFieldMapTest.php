...
it('returns sections for every registered excursion type', function (): void {
    $types = [
...
it('returns empty array for unknown type', function (): void {
    $sections = $this->fieldMap->getSections('unknown_type');
    expect($sections)->toBeEmpty();
});

it('includes general fields for peripatos', function (): void {
...
it('includes date fields for peripatos with single date', function (): void {
    $sections = $this->fieldMap->getSections('peripatos');
...
it('does not include participation section for peripatos', function (): void {
    $sections = $this->fieldMap->getSections('peripatos');

    expect($sections)->not->toHaveKey('participation');
});

it('includes general fields for hmerisiaxoris', function (): void {
...
it('includes participation fields for hmerisiaxoris', function (): void {
    $sections = $this->fieldMap->getSections('hmerisiaxoris');
...
it('includes multi-day date fields for pollesesjot', function (): void {
...
it('includes onoma_arxigos in participation for pollesesjot', function (): void {
    $sections = $this->fieldMap->getSections('pollesesjot');

    expect($sections['participation'])->toHaveKey('onoma_arxigos');
});

it('includes all time fields for ekp_exotiko', function (): void {
...
it('includes insurance and agency fields for ekp_exotiko', function (): void {
    $sections = $this->fieldMap->getSections('ekp_exotiko');

    expect($sections['general'])->toHaveKeys(['asf_symbolaio', 'praji_epilogi_praktoreiou', 'ar_pr_anartisisprok']);
});

it('includes program fields for programma_esoteriko', function (): void {
...
it('includes erasmus-specific fields for erasmus2', function (): void {
...
it('includes erasmus list fields in participation for erasmus2', function (): void {
    $sections = $this->fieldMap->getSections('erasmus2');
...
it('includes mathimata for ekp_esoteriko general section', function (): void {
    $sections = $this->fieldMap->getSections('ekp_esoteriko');

    expect($sections['general'])->toHaveKey('mathimata');
});

it('includes tmimata in participation for ekp_esoteriko', function (): void {
    $sections = $this->fieldMap->getSections('ekp_esoteriko');

    expect($sections['participation'])->toHaveKey('tmimata');
});

it('includes tmimata in participation for didaktikes', function (): void {
    $sections = $this->fieldMap->getSections('didaktikes');

    expect($sections['participation'])->toHaveKey('tmimata');
});

it('does not include multi-day dates for didaktikes', function (): void {
    $sections = $this->fieldMap->getSections('didaktikes');
...
it('returns signer fields', function (): void {
...
it('returns field definition for known field', function (): void {
...
it('returns null for unknown field', function (): void {
    $def = $this->fieldMap->getFieldDefinition('nonexistent_field');
    expect($def)->toBeNull();
});

it('returns correct section labels', function (): void {
...
it('identifies date fields correctly', function (): void {
...
it('identifies time fields correctly', function (): void {
...
it('checks section existence correctly', function (): void {
...
it('returns types with sections mapping', function (): void {
...
it('returns correct required fields for peripatos', function (): void {
...
it('returns correct required fields for hmerisiaxoris', function (): void {
...
it('returns validation rules for peripatos', function (): void {
...
it('returns validation rules for erasmus2 with erasmus fields', function (): void {
...
it('makes date fields required in validation rules when applicable', function (): void {
    $rules = $this->fieldMap->getValidationRules('peripatos');

    expect($rules['hmera_ekdromis_anaxorisis'])->toContain('required');
});

it('makes core participation fields required in validation', function (): void {
